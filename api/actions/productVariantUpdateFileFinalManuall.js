...
export async function run({ params, logger, api, connections }) {
  ...
  const productsVariants = await getProductVariants(api, logger, itemTimeStamp);
 ...
 */
const getProductVariants = async (api, logger, itemTimeStamp) => {
  const allRecords = [];
  ...
  try {
    let records = await api.erpItem.findMany({
      ...
      filter: {
        itemTimeStamp: {
          greaterThanOrEqual: itemTimeStamp,
        },
      },
    });
