...
const AddDraftOrderPage = () => {
  ...
  const [{ data: draftOrderData, fetching: draftOrderFetching, error: draftOrderError }, draftOrderAdd] = useGlobalAction(api.createDraftOrder);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const requestData = {
      ...
      customer: {
        ...
        first_name: selectedCustomer?.firstName || null,
        last_name: selectedCustomer?.lastName || null,
  ...
  //   return productData && productData.flatMap(product =>
    ...
    if (selectedResources) {
      ...
      setSelectedProducts((prevSelectedProducts) => {
        ...
        const updatedProducts = selectedProducts.map((product) => {
          ...
          const lineItemTags = product.tags; 
  ...
  }, [selectedResources,productItemTags]);
  ...
  useEffect(() => {
    if ( selectedProducts.length && selectedCustomer.id) {
      setIsEditing(true);
    }
  }, [ tags, selectedCustomer]);
  ...
  useEffect(() => {
    ...
    const markup = Object.keys(groupedProducts).map((handle,index) => {
     ...
     const selectableRows = filteredRowsForStore.filter(({ disabled }) => !disabled);
      const rowRange = [
            ...
            <Card roundedAbove="sm">
              <BlockStack gap="400">
                ...
                <Card roundedAbove="sm">
                  ...
                  <TextComponent 
                        textTitle={
                          showProducts && selectedProducts.length && subtotal 
                            ? totalTaxes !== 0.0 
                              ? "(Included)" 
                              : "Not calculated" 
                            : ""
                        ...
                        textAs="h6" 
                      />
                    <TextComponent textTitle={showProducts && selectedProducts.length && subtotal ? `₹${totalTaxes}` : "₹0.00"} textVariant="bodyMd" textAs="p" />
                  </InlineStack>
              ...
              {showProducts && selectedProducts.length ? (
                ...
                <Box paddingBlockStart="300">
                  <Text as="h6" variant="headingSm">
                    Add a product to calculate total.
                  </Text>
                </Box>
              )}
            </Card>
