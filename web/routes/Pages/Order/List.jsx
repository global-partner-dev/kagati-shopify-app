...
const OrderListPage = () => {
  ...
  const [splitcursor, setSplitCursor] = useState({ first: NumberOnPage });
  ...
  const [{ data: splitsOrder, fetching: splitsOrderFetching, error: splitsOrderError }] = useFindMany(api.khagatiOrderSplit, {
  ...
  const [{ data: orders, fetching: ordersFetching, error: ordersError }, refetch] = useFindMany(api.khagatiOrder, {
    live: true,
    ...cursor,
    select: {
      id: true,
    ...
    ...searchParams,
    filter: {
      ...(selectStoreCode ? {} : filterBy),
      ...(selectStoreCode ? {} : customerFilter),
    },
    ...(selectStoreCode ? {} : { sort: convertSortOrder(sortSelected) }),

  });

  console.log("ordersFetching--->", ordersFetching)
  ...
  const OrderRow = ({ order, selected, navigate, index }) => {
    const { id, name, customer, tags, currency, currentTotalPrice, financialStatus, fulfillmentStatus, lineItems, createdAt, note, orderId, noteAttributes, title } = order;
    const convertDate = dateFormatter.format(createdAt);
    console.log("heree---> orderData")
    ...
    const [{ data: ordersss, fetching: orderFetching, error: orderError }] = useFindOne(api.shopifyOrder, orderId, {
      select: {
        id: true,
        shippingLines: {
          edges: {
    ...
    // const shippingMethodTitle = matchingShippingMethod ? matchingShippingMethod.title : null;
      ...
      >
        ...
        <IndexTable.Cell>
          {splits && splits.length ?
            (<Badge
              tone={splits[0].orderStatus === "cancel" ? "critical" : splits[0].orderStatus === "delivered" ? "success" : "attention"}
              progress={
                fulfillmentStatus === null ? "incomplete" : "complete"
              }
            >
              {splits[0].orderStatus}
            </Badge>) : null
          }
        </IndexTable.Cell>
        <IndexTable.Cell>
          <Text as="span" variant="bodyMd">
            {lineItems.filter((item) => item.currentQuantity > 0).reduce((sum, item) => sum + item.currentQuantity, 0)}{" "}
            {lineItems.filter((item) => item.currentQuantity > 0).reduce((sum, item) => sum + item.currentQuantity, 0) > 1 ? "items" : "item"}
          </Text>
        </IndexTable.Cell>
        {/* <IndexTable.Cell>
        ...
        {ordersss?.shippingLines?.edges[0]?.node?.title ? (
          <IndexTable.Cell>
            <Badge tone="warning">
              {ordersss?.shippingLines?.edges[0]?.node?.title}
            </Badge>
  ...
  const handleLocations = (value) => {
    ...
    setSplitCursor({ first: NumberOnPage })
  };
  ...
  // useEffect(() => {
  //   if (selectStoreCode || filterBy) {
  //     fetchSplits(selectStoreCode);
  //   } else {
  //     setSplitsData([]);
  //   }
  // }, [selectStoreCode, filterBy,splitcursor,sortSelected]);
  ...
  useEffect(() => {
    if (locationsFetching || storesFetching || splitsOrderFetching || ordersFetching) {
      shopify.loading(true)
    } else {
      shopify.loading(false)
    }
  }, [locationsFetching, storesFetching, splitsOrderFetching, ordersFetching]);
  function generateBlogPost() {
    shopify.loading(true)
    // Handle generating
    shopify.toast.show(`${shopify.config.shop}`);
    setTimeout(() => {
      shopify.loading(false)
    }, 5000);
  }
  ...
  return (
    locationsFetching || storesFetching || splitsOrderFetching || ordersFetching ? <Skeleton /> :
      <Page
        fullWidth
        compactTitle
        title="Orders:"
      ...
      >
        {selectStoreCode == "" && orderDatas ? <TableComponent
