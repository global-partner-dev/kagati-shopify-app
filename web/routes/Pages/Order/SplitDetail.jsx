...
const OrderDetailPage = () => {
      ...
      if (data.lineItems && data.lineItems.edges) {
          ...
          for (const item of lineItems) {
              ...
              if(node.currentQuantity != 0){
                totalQuantity += parseInt( node.quantity); 
                totalPrice += parseFloat(node.price)*parseInt( node.quantity);
    ...
    useEffect(() => {
        if (data?.length && splits?.length) {
            ...
            const filteredLineItems = getData.lineItems
                            ...
                            <Card roundedAbove="sm">
                                <BlockStack gap="400">
                                    <InlineStack align="space-between">
                                        ...
                                        >
                                            {priceOrderData?.fulfillmentStatus === null ? "Unfulfilled" : "Fulfilled"} ({splits && splits[0]?.lineItems?.length})
                                        </Badge>
                                    </InlineStack>
                                    <Card>
