...
import { api } from "../../../api";
import { formaterDate, objectEqual, formaterTime } from "../../../util/commonFunctions";
      ...
      {orderData ? (
        <Page
          ...
          titleMetadata={<InlineStack gap="200">
            ...
            >
              {priceOrderData?.fulfillmentStatus === null ? "Unfulfilled" : priceOrderData.fulfillmentStatus}
            </Badge>
          </InlineStack>}
        ...
        >
          <InlineGrid columns={{ xs: 1, md: "2fr 1.5fr" }} gap="400">
            <BlockStack gap="400">
              <Card roundedAbove="sm">
                <BlockStack gap="400">
                  <InlineStack align="space-between">
                    ...
                    >
                      {orderData?.fulfillmentStatus === null ? "Unfulfilled" : "Fulfilled"} ({orderData?.lineItems?.length})
                    </Badge>
                  </InlineStack>
                  <Card>
                    <Box paddingBlockEnd="400">
              ...
              <List gap="extraTight">
                {/* <InlineStack align="space-between">
                  <List.Item>Alex P updated the customer for this order.</List.Item>
                  <Text as="span">{formaterTime(orderData?.createdAt)}</Text>
                </InlineStack> */}
                <InlineStack align="space-between">
                  <List.Item>You manually marked {`${orderData?.currency === "INR" ? "₹" : "$"}${orderData?.currentTotalPrice}`} as paid for this order.</List.Item>
                  <Text as="span">{formaterTime(orderData?.createdAt)}</Text>
                </InlineStack>
                <InlineStack align="space-between">
                  <List.Item>Confirmation {orderData?.name} was generated for this order.</List.Item>
                  <Text as="span">{formaterTime(orderData?.createdAt)}</Text>
                </InlineStack>
                <InlineStack align="space-between">
                  <List.Item>{`${orderData?.customer?.firstName} ${orderData?.customer?.lastName}`} created this order from draft {orderData?.name}.</List.Item>
                  <Text as="span">{formaterTime(orderData?.createdAt)}</Text>
                </InlineStack>
              </List>
            </BlockStack>
